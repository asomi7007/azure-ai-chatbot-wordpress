...
### 🔧 버그 수정
- **Agent 모드 Client Secret 복호화 오류 수정**: OAuth 자동 설정 후 Agent 모드 연결 테스트 시 "Client Secret을 복호화할 수 없습니다" 오류 해결.
  - **원인**: `ajax_save_existing_config()`에서 이미 복호화된 `$this->client_secret` 값을 암호화 없이 저장하여, 이후 복호화 시도 시 실패.
  - **수정**: OAuth에서 가져온 평문 Client Secret을 저장 전 암호화 처리 추가.
  - **관련 파일**: `includes/class-azure-oauth.php` (2462-2466줄)
- **테스트 연결 코드 간소화**: 불필요한 복호화 로직 제거, `load_options()`에서 이미 복호화된 값 직접 사용.
  - **관련 파일**: `azure-ai-chatbot.php` (992-1001줄)
